...
public class MiscPolyGeom {
	 ...
	 */
	public static WrapList<Point> buildUnitPoly (Point c1, float r1, Point c2, float r2) {
		...
		WrapList<Point> poly = new WrapList<Point>();
	...
	public static int calcWindingNum (Point point, WrapList<Point> poly) {
		...
		for (int i=0; i < poly.size(); i++) {
			if (poly.get(i).y <= point.y && poly.get(i+1).y > point.y) {
				...
				float handedness = MiscGeom.cross(poly.get(i+1), poly.get(i), point, poly.get(i));
				if (handedness > 0) {
					windingNum++;
				} else if (handedness == 0) {
					return Integer.MIN_VALUE;
				}
			} else if (poly.get(i).y > point.y && poly.get(i+1).y <= point.y) {
	...
	public static boolean nzPointInPoly (Point point, WrapList<Point> poly) {
		return calcWindingNum(point, poly) != 0;
	}
	
	public static boolean jctPointInPoly (Point point, WrapList<Point> poly) {
